...
import { containsUnsafeLatexCommand } from "@/latex/inlineLatex";
...
function importTrackedBlocks(body: string, definitions: BlockDefinition[]) {
  ...
  const trackedBlockRegex =
    /% editortex:block (?<encodedMetadata>[^\n]+)\n(?<latex>[\s\S]*?)\n% editortex:endblock/g;
...
function createBlockFromTrackedLatex(
  ...
  const parsedCommand =
    metadata.type === "custom-cover"
      ...
      : parseCommandArguments(trimmedLatex, trimmedLatex.indexOf(`\\${metadata.variableName}`) + metadata.variableName.length + 1, definition.fields.length);
...
function extractCoverTitle(source: string) {
  ...
  const title = extractCoverNodeText(source, "T[ÍI]TULO PRINCIPAL");
  const subtitle = extractCoverNodeText(source, "SUBT[ÍI]TULO");
