  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2.5">
        ...
        />
        <ModeButton active={mode === "template-only"} label="Novo com template" onClick={() => setMode("template-only")} />
      </div>
      ...
      {error ? <span className="mt-2 block rounded-xl border border-[#FB7185]/40 bg-[#F43F5E]/75 px-2 py-1 text-xs font-semibold text-white shadow-[0_0_24px_rgba(244,63,94,0.18)]">{error}</span> : null}
    </div>
  );
}
