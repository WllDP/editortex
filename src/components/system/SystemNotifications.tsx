  ...
  return (
    <div className="pointer-events-none fixed right-5 top-5 z-[90] flex w-[min(380px,calc(100vw-40px))] flex-col gap-2">
      {notifications.map((notification) => (
        <NotificationToast key={notification.id} notification={notification} onDismiss={() => dismiss(notification.id)} />
      ))}
    </div>
  );
}
  ...
  return (
    <div
      className={cn(
        ...
        notification.isClosing ? "translate-x-[120%] opacity-0" : "translate-x-0 opacity-100 animate-[toast-slide-in_300ms_ease-out]",
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {notification.message ? <p className="mt-0.5 text-xs font-medium leading-5 text-[#CBD5E1]">{notification.message}</p> : null}
      </div>
      <button
        type="button"
